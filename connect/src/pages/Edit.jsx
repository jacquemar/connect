...
const handleBanniereUpload = async (event) => {
    ...
    try {
        const response = await axios.post('http://localhost:3000/upload/banniere', formData);
...
const handlePhotoProfilUpload = async (event) => {
    ...
    try {
        const response = await axios.post('http://localhost:3000/upload/photoProfil', formData);
        const url = response.data.url;
...
const handlePhoneNumberChange = (event) => {
	...
	const handleSubmit = async (event) => {
		...
		try {
			...
			const response = await axios.put(`http://localhost:3000/edit/profil/${userId}`, {
				photoProfilURL,
				banniereURL,
				nomComplet,
				titre,
